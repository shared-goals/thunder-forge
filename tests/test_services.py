...
def test_generate_systemd_unit_with_environment_and_user() -> None:
    spec = LaunchdServiceSpec(
        name="sample",
        label="com.thunder-forge.sample",
        program_arguments=["/usr/local/bin/sample", "serve"],
    ...
    assert "[Unit]" in unit
    assert "Description=Thunder Forge sample (com.thunder-forge.sample)" in unit
    assert "[Service]" in unit
    assert "User=shag" in unit
    assert "WorkingDirectory=/tmp/thunder-forge" in unit
    assert "Environment=HOME=/home/shag" in unit
    assert "Environment=PATH=/usr/bin:/bin" in unit
    assert "ExecStart=/usr/local/bin/sample serve" in unit
    assert "StandardOutput=append:/tmp/sample.out" in unit
...
def test_systemd_commands_can_prompt_for_install_repair() -> None:
    commands = systemd_commands(
        service_name="com.thunder-forge.sample.service",
        staging_unit_path="/tmp/com.thunder-forge.sample.service",
        unit_path="/etc/systemd/system/com.thunder-forge.sample.service",
        setup_command="mkdir -p /tmp",
        interactive_sudo=True,
    )

    assert commands[0] == "mkdir -p /tmp"
    assert "password prompt: host=%h method=sudo user=%p" in commands[1]
    joined = "\n".join(commands)
    assert "/bin/systemctl stop com.thunder-forge.sample.service" in joined
    assert "/usr/bin/install -o root -g root -m 644" in joined
    assert "/bin/systemctl daemon-reload" in joined
    assert "/bin/systemctl enable --now com.thunder-forge.sample.service" in joined
    assert "/bin/systemctl restart com.thunder-forge.sample.service" in joined
    assert "/bin/systemctl is-active --quiet com.thunder-forge.sample.service" in joined


def test_systemd_commands_can_prompt_through_admin_user() -> None:
    commands = systemd_commands(
        service_name="com.thunder-forge.sample.service",
        staging_unit_path="/tmp/com.thunder-forge.sample.service",
        unit_path="/etc/systemd/system/com.thunder-forge.sample.service",
    ...
    joined = "\n".join(commands)
    assert "password prompt: host=%h method=su user=serpo" in commands[1]
    assert "reason=manage Thunder Forge service com.thunder-forge.sample.service" in commands[1]
    assert sum("/usr/bin/su - serpo -c" in command for command in commands) == 1
    assert "password prompt: host=%h method=sudo user=serpo" in joined
    assert "/bin/systemctl enable --now com.thunder-forge.sample.service" in joined
    assert commands[-1] == "true"
...
def test_gateway_daemon_setup_generates_combined_sudoers(tmp_path: Path, monkeypatch) -> None:
    import thunder_forge.cluster.gateway as gateway_module

    monkeypatch.setattr(gateway_module.platform, "system", lambda: "Darwin")
...
def test_gateway_daemon_setup_generates_systemd_sudoers_on_linux(tmp_path: Path, monkeypatch) -> None:
    import thunder_forge.cluster.gateway as gateway_module

    monkeypatch.setattr(gateway_module.platform, "system", lambda: "Linux")
    monkeypatch.setattr(gateway_module, "_linux_shell_program", lambda: "/bin/zsh")
    ...
    assert result.sudoers_path == "/etc/sudoers.d/thunder-forge"
    assert "Cmnd_Alias TF_OLLA_40115_SYSTEMD" in result.script_content
...
def test_gateway_daemon_setup_apply_verifies_with_systemd_on_linux(tmp_path: Path, monkeypatch) -> None:
    import thunder_forge.cluster.gateway as gateway_module
...
def test_run_olla_service_restart_daemon_apply_writes_staging_before_install(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(olla_module.platform, "system", lambda: "Darwin")
...
def test_run_edge_service_restart_dry_run_describes_frontend_launch_daemon(tmp_path: Path, monkeypatch) -> None:
...
def test_run_olla_service_restart_systemd_apply_writes_staging_before_install(tmp_path: Path, monkeypatch) -> None:
    import thunder_forge.cluster.olla as olla_module

    command_batches: list[list[str]] = []
    written_files: list[tuple[str, str]] = []

    monkeypatch.setattr(olla_module.platform, "system", lambda: "Linux")
    ...
    result = run_olla_service_restart(repo_root=tmp_path, apply=True, manager="systemd", timeout=12, user="shag")

    assert result.ok
    assert result.label == "com.thunder-forge.olla-40115.service"
    assert written_files[0][0] == str(tmp_path / ".tmp/run/com.thunder-forge.olla-40115.service")
    assert "ExecStart=" in written_files[0][1]
    assert command_batches[0] == [f"mkdir -p {tmp_path}/.tmp/run {tmp_path}/logs"]
    assert "/bin/systemctl stop com.thunder-forge.olla-40115.service" in command_batches[1][0]
    assert "/bin/systemctl daemon-reload" in command_batches[1][2]
    assert "/bin/systemctl enable --now com.thunder-forge.olla-40115.service" in command_batches[1][3]
    assert command_batches[2] == [
        "/usr/bin/sudo -n /bin/systemctl is-active --quiet com.thunder-forge.olla-40115.service"
    ]


def test_run_edge_service_restart_systemd_apply_writes_staging_before_install(tmp_path: Path, monkeypatch) -> None:
    import thunder_forge.cluster.edge as edge_module

    command_batches: list[list[str]] = []
    written_files: list[tuple[str, str]] = []

    monkeypatch.setattr(edge_module.platform, "system", lambda: "Linux")
    ...
    monkeypatch.setattr(edge_module, "run_local_commands", fake_run_local_commands)
    monkeypatch.setattr(edge_module, "write_local_file", fake_write_local_file)
    monkeypatch.setattr(edge_module, "_wait_edge_healthy", lambda base_url, **kwargs: True)

    result = run_edge_service_restart(repo_root=tmp_path, apply=True, manager="systemd", timeout=12, user="shag")

    assert result.ok
    assert result.label == "com.thunder-forge.edge-40116.service"
    assert written_files[0][0] == str(tmp_path / ".tmp/run/com.thunder-forge.edge-40116.service")
    assert "ExecStart=" in written_files[0][1]
    assert command_batches[0] == [f"mkdir -p {tmp_path}/.tmp/run {tmp_path}/logs"]
    assert "/bin/systemctl stop com.thunder-forge.edge-40116.service" in command_batches[1][0]
    assert "/bin/systemctl daemon-reload" in command_batches[1][2]
    assert "/bin/systemctl enable --now com.thunder-forge.edge-40116.service" in command_batches[1][3]
    assert command_batches[2] == [
        "/usr/bin/sudo -n /bin/systemctl is-active --quiet com.thunder-forge.edge-40116.service"
    ]
