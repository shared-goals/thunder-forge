...
with request_rows as (
    select
        ...
        client_id,
        coalesce(node_name, regexp_extract(olla_endpoint, '^([^-]+)-omlx-', 1), '') as node_name,
        model,
...
where node_name <> ''
group by node_name
order by requests desc, node_name;
...
select
    ...
    count(*) as requests
from request_rows
where node_name <> '' and model <> ''
group by node_name, model
order by node_name, requests desc, model;
...
from request_rows
where node_name <> ''
group by node_name, strftime(ts, '%H')
